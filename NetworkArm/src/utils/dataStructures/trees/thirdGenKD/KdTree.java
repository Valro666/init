...
public class KdTree<T> extends KdNode<T> {
    ...
    /**
     * Creation of Tree where each Node stores (points,data)
     * 
     * @param dimensions nbOfDimension for each 'point'
     * @param bucketCapacity max points for a KdNode, then split.
     */
    ...
    public NearestNeighborIterator<T> getNearestNeighborIterator(double[] searchPoint, int maxPointsReturned, DistanceFunction distanceFunction) {
        return new NearestNeighborIterator<T>(this, searchPoint,
        		maxPointsReturned, distanceFunction);
    }
    ...
    public MaxHeap<T> findNearestNeighbors(double[] searchPoint, int maxPointsReturned,
        ...
        while (pendingPaths.size() > 0 && 
            ...
            nearestNeighborSearchStep(pendingPaths, evaluatedPoints,
            		pointsRemaining, distanceFunction, searchPoint);
        }
